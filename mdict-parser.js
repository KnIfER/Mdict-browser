...
(function (root, factory) {
   ...
   *   1. Search kBiL to locate keyword block containing the given key.
  ...
  function parse_mdict(file, ext) {
    ...
    var attrs = {},     // storing dictionary attributes
        ...
        _decryptors  = [false, false],
                        // [keyword_header_decryptor, k_b_infoL_decryptor], only k_b_infoL_decryptor is supported
    ...
    /**
     * Read keyword summary at key block header before key block info section , just a set of Numbers
     */
    function read_keyword_summary(input, offset) {
      ...
      return {
        ...
        k_b_info_decomp_len: _v2 && scanner.readNum(),  // Ver >= 2.0 only
        k_b_info_comp_len:   scanner.readNum(),
        k_b_len:       scanner.readNum(),
        chksum:               scanner.checksum_v2(),// 前5 个数的alder32 checksum
        // extra field
    ...
    /**
     * Read keyword block info
     */
    function read_k_b_infoL(input, keyword_summary) {
      var scanner = Scanner(input).readBlock(keyword_summary.k_b_info_comp_len, keyword_summary.k_b_info_decomp_len, _decryptors[1]),
          k_b_infoL = Array(keyword_summary.num_blocks),
          offset = 0,acc = 0;
      ...
      for (var i = 0, size; i < keyword_summary.num_blocks; i++) {
        k_b_infoL[i] = {
          ...
          // extra fields
          offset: offset,     // this is key block compressed size accumulator.
          num_entries_acc:acc,
          index: i            // this index
        };
        offset += size;
        acc+=k_b_infoL[i].num_entries;
      }
      return spreadus(keyword_summary, k_b_infoL);
    }
...
//        console.log(scanner.readNum(), scanner.readText());
     ...
     * @param k_b_infoL array of keyword index
     * @param delay time to delay for scanning key table
     */
    function willScanKeyTable(slicedKeyBlock, num_entries, k_b_infoL, delay) {
      slicedKeyBlock.delay(delay).then(function (input) {
        common.log('scan key table...');
        var scanner = Scanner(input);
        for (var i = 0, size = k_b_infoL.length; i < size; i++) {
          read_key_block(scanner, k_b_infoL[i]);
        }
    ...
    function posReduce(arr, pos) {
        var len = arr.length;
        if (len > 1) {
            len = len >> 1;
            //console.log("==posReduce_degbug: "+len);
            //console.log("    degbug: "+pos);
            //console.log("    degbug: "+arr[len - 1].num_entries_acc);
            return pos > arr[len].num_entries_acc
                ? posReduce(arr.slice(len), pos)
                : posReduce(arr.slice(0, len), pos);
        } else {
            return arr[0];
        }
    }
    ...
    function fillIt(acc,base,len){
        ...
        for(var i=0;i<len;i++){
            ...
            item.className="cp";
            item.onclick = p;
            item.innerText = _cached_keys.list[base-acc+i];
            if(lastSelection==base+i){
                lastTarget = item;
    ...
    function seekVanguard(phrase) {
      ...
      if (phrase <= _adaptKey(kdx.last_word)) {
        ...
        while (prev = kBiL[index]) {//什么乱七八糟的，走着瞧！
    ...
    return _slice(pos, 4).exec(read_file_head).spread(function(len) {
      ...
      pos += header_remain_len;                   // start of key block info
      ...
      return _slice(pos, keyword_summary.k_b_info_comp_len)
                .exec(read_k_b_infoL, keyword_summary);

    }).spread(function (keyword_summary, k_b_infoL) {
      pos += keyword_summary.k_b_info_comp_len;  // start of keyword block in keyword section
        common.log("start of key block: ",pos);
        common.log("k_b_info_comp_len: ",keyword_summary.k_b_info_comp_len);
      slicedKeyBlock = _slice(pos, keyword_summary.k_b_len);
        LOOKUP[ext].k_b_cap = pos;
        LOOKUP[ext].k_b_ln = keyword_summary.k_b_len;
      /*
      ...
      willScanKeyTable(slicedKeyBlock, keyword_summary.num_entries, k_b_infoL, 00);
      // */

      pos += keyword_summary.k_b_len;    // start of record section
      ...
      common.log("【2】record_summary:"+record_summary);
  ...
  return function load(files) {
      ...
      Array.prototype.forEach.call(files, function(f) {
        ...
        resources.push(resources[f.name] = parse_mdict(f, ext));
      });
